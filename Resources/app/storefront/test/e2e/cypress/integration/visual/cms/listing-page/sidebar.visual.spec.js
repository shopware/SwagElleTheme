...
describe('CMS: Listing Page', { tags: ['@visual', '@cms'] }, () => {
    ...
    function createSampleListingPageWithSidebar() {
    ...
    it('@visual @cms: Create listing page with a sidebar', () => {
        ...
        cy.get('.nav-main-toggle-btn').click();
        cy.get('.navigation-offcanvas').find('.navigation-offcanvas-link').click();
        cy.get('.breadcrumb-title').contains('Sidebar Listing Categorian');
        cy.get('.nav-main-toggle-btn').click();
        cy.get('.navigation-offcanvas .is-home-link.navigation-offcanvas-link').contains('Show all categories');
        cy.get('.navigation-offcanvas .navigation-offcanvas-headline').contains('Sidebar Listing Categorian');
        cy.takeSnapshot('[Listing] Navigation Offcanvas', '.navigation-offcanvas', {widths: [375, 768]});

        cy.visit('/');
        cy.get('.filter-panel-wrapper-toggle').click();
        cy.get('.offcanvas-filter').should('be.visible');
